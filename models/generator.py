...
class Generator(nn.Module):
    def __init__(self, device, theta=0.8, iteration=4, ae_lambda=[0.6, 0.8, 1.0]):
    ...
    def forward(self, input):
        ...
        mask = Variable(torch.ones(batch_size, 1, row, col)).to(self.device) / 2.
        h = Variable(torch.zeros(batch_size, 32, row, col)).to(self.device)
        c = Variable(torch.zeros(batch_size, 32, row, col)).to(self.device)
        mask_list = []
